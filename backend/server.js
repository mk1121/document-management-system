...
app.post('/api/v1/documents/sync', async (req, res) => {
  const { transactionId, metadata, attachments, username } = req.body;
  ...
  try {
    ...
    console.log('Inserted Patient ID:', masterId); // Debug log
    ...
    const sqlImage = `
      ...
      VALUES (:mid, :seq, :mime, :data, TO_DATE(:nextApp, 'YYYY-MM-DD'), :userId)
    `;
...
app.get('/api/v1/patients/:id/images', async (req, res) => {
  ...
  try {
    ...
    for (const row of result.rows) {
      // row[0]=FILE_ID, row[1]=SEQ, row[2]=MIME, row[3]=BLOB, row[4]=NEXT_APP
      const fileId = row[0];
      const seq = row[1];
      const mime = row[2];
      const lob = row[3];
...
app.post('/api/v1/patients/:id/images', async (req, res) => {
  const { id } = req.params;
  const { images, username } = req.body; // Expects array of { data: "base64...", mimeType: "..." }
