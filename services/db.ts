...
/**
 * Gets a single DocMaster by ID.
 */
export const getDocMaster = async (id: string, username: string): Promise<DocMaster | null> => {
  const db = await openDB(username);
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([MASTER_STORE], 'readonly');
    const store = transaction.objectStore(MASTER_STORE);
...
export const saveDocMaster = async (master: DocMaster, username: string): Promise<void> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const store = transaction.objectStore(MASTER_STORE);
    const request = store.put(master);
...
/**
 * Gets DocDetails by masterId.
 */
export const getDocDetails = async (masterId: string, username: string): Promise<DocDetail[]> => {
  const db = await openDB(username);
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([DETAIL_STORE], 'readonly');
    const store = transaction.objectStore(DETAIL_STORE);
 ...
 */
export const getPendingCount = async (username: string): Promise<number> => {
  const docs = await getPendingDocuments(username);
  return docs.length;
};
 ...
 */
export const getFailedCount = async (username: string): Promise<number> => {
  const docs = await getFailedDocuments(username);
  return docs.length;
};

/**
 * Saves multiple DocDetails.
 */
export const saveDocDetails = async (_docId: string, details: DocDetail[], username: string): Promise<void> => {
  for (const detail of details) {
    await saveDocDetail(detail, username);
  }
};
 ...
 */
export const clearAllData = async (username: string): Promise<void> => {
  await clearDatabase(username);
};

/**
 * Gets unsynced documents (pending or failed).
 */
export const getUnsyncedDocs = async (username: string): Promise<DocMaster[]> => {
  const pending = await getPendingDocuments(username);
  const failed = await getFailedDocuments(username);
  return [...pending, ...failed];
};

/**
 * Updates the sync status of a document.
 */
export const updateSyncStatus = async (masterId: string, status: 'pending' | 'synced' | 'failed', username: string) => {
  const db = await openDB(username);
  return new Promise<void>((resolve, reject) => {
    const tx = db.transaction([MASTER_STORE], 'readwrite');
    const store = tx.objectStore(MASTER_STORE);
...
/**
 * Retries failed syncs by marking them as pending.
 */
export const retryFailedSyncs = async (username: string): Promise<void> => {
  const failed = await getFailedDocuments(username);
  for (const doc of failed) {
    await updateSyncStatus(doc.id, 'pending', username);
  }
};
