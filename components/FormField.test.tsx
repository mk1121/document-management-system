...
describe('FormField', () => {
  ...
  describe('Date Input', () => {
    it('sets inputMode to numeric', () => {
      render(<FormField label='Date' value='' onChange={jest.fn()} type='date' />);
      expect(screen.getByRole('textbox')).toHaveAttribute('inputMode', 'numeric');
    });

    it('formats ISO value to DD/MM/YYYY', () => {
      render(<FormField label='Date' value='2023-10-25' onChange={jest.fn()} type='date' />);
    ...
    it('syncs external value change back to display format', () => {
      ...
      const { rerender } = render(
        <FormField label='Date' value='2023-01-01' onChange={handleChange} type='date' />,
      );
